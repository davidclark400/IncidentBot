...
/// <summary>
/// Collects profile-scoped Kafka metrics through Grafana's read-only data-source query endpoint.
    ...
    ICredentialProvider credentials) : IIncidentEvidenceConnector
{
    ...
    public string Source => EvidenceSourceRegistry.Kafka;
    ...
    {
        var configuration = context.Profile.Kafka;
        if (configuration is null)
        {
            return Task.FromResult(ConnectorResult.Excluded(Source));
        }

        var transport = evidenceSources.For(Source);
        if (!string.Equals(transport.Mode, "api", StringComparison.Ordinal))
        {
            return Task.FromResult(ConnectorResult.Unavailable(
                Source,
                0,
                "Kafka evidence supports only Grafana API transport; MCP mode is not supported."));
        }

        return ConnectorUtilities.ExecuteAsync(
            Source,
    ...
    private async Task<ConnectorResult> CollectNativeAsync(
        InvestigationContext context,
        EvidenceScope scope,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var dashboardUrl = DashboardUrl(transport, context.Profile.Id, scope);
        ...
        for (var batchIndex = 0; batchIndex < batches.Length; batchIndex++)
        {
            ...
            catch (Exception exception)
            {
                ...
                AddDiagnostic(
                    ...
                    + ConnectorUtilities.Truncate(exception.Message, 180));
            }
        }

        var batchDiagnostic = ConnectorUtilities.CombineDiagnostics(
            budget.Diagnostic,
        ...
        var diagnostic = ConnectorUtilities.CombineDiagnostics(
            ...
            diagnostics.Count == 0 ? null : string.Join("; ", diagnostics),
            itemsTruncated
                ? $"Source item limit {itemLimit} truncated Kafka findings, timeline entries, or links."
                : null);
        return new ConnectorResult(
            Source,
            partial ? SourceHealth.Partial : SourceHealth.Complete,
            rankedFindings.Take(itemLimit).ToArray(),
            orderedTimeline.Take(itemLimit).ToArray(),
        ...
        foreach (var planned in batch)
        {
            ...
            if (ResultError(result) is { } resultError)
            {
                ...
                AddDiagnostic(
                    ...
                    + ConnectorUtilities.Truncate(resultError, 160));
                continue;
            }
        ...
        KafkaPlannedMetric metric)
    {
                     ...
                     .Take(MaximumLabelsPerSeries))
        {
            ...
            bounded.TryAdd(
                ConnectorUtilities.Truncate(label.Key, 80),
                ConnectorUtilities.Truncate(label.Value, valueLimit));
        }
        return bounded;
    }
        ...
        out bool timestampSupported)
    {
        ...
        // temporal precision and make an unsound causal event.
    ...
    private static int QueryIntervalMilliseconds(EvidenceScope scope)
    {
    ...
    private static string DashboardUrl(
        ...
        EvidenceScope scope)
    {
        var uid = KafkaDashboardIdentity.Uid(profileId);
        return ConnectorUtilities.Url(
            transport,
    ...
    private static string FieldType(JsonElement field) =>
        ConnectorUtilities.Text(field, "type", "").ToLowerInvariant();

    private static string FieldName(JsonElement field) =>
        ConnectorUtilities.Truncate(ConnectorUtilities.Text(field, "name", "value"), 120);
    ...
    private static void AddDiagnostic(ICollection<string> diagnostics, string diagnostic)
    {
        ...
        var bounded = ConnectorUtilities.Truncate(diagnostic, 220);
