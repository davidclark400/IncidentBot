...
public sealed class KafkaOnboardingValidator(KafkaDashboardGenerator dashboards)
{
        ...
        ICollection<string> errors)
    {
        ...
        foreach (var resource in inventory.Resources.Where(resource => resource.Kind == kind))
        {
            ...
            if (mapped.TryGetValue(resource.Name, out var mapping)
                && allowlist.Contains(mapping.ProfileResource))
            {
                continue;
            }
            ...
            var evidence = resource.Evidence.OrderBy(item => item.File, StringComparer.Ordinal)
                ...
                .FirstOrDefault();
            errors.Add(
                $"Inventory Kafka {kind} '{resource.Name}' is absent from the profile allowlist"
                + (mapped.ContainsKey(resource.Name) ? " and has no mapping to an allowlisted profile resource" : " and has no reviewed resource mapping")
        ...
        ICollection<string> errors)
    {
        ...
        foreach (var mapping in mappings)
        {
            ...
            if (!allowlists[mapping.Kind].Contains(mapping.ProfileResource))
            {
                errors.Add(
                    $"Kafka resource mapping for {mapping.Kind} '{mapping.InventoryResource}' at {evidence} "
                    + $"targets '{mapping.ProfileResource}', which is outside the profile allowlist.");
            }
        }
    }
}
