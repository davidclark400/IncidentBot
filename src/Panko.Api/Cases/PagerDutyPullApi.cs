...
public static class PagerDutyPullApi
{
    public static IEndpointRouteBuilder MapPagerDutyPullApi(this IEndpointRouteBuilder endpoints)
    {
        ...
        endpoints.MapPost("/api/pagerduty/incidents/{id}/trigger", TriggerAsync)
            ...
            .Produces<CaseTriggerResult>(StatusCodes.Status202Accepted)
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            var scope = authorization.ResolveScope(context.User);
            var serviceIds = authorization.AuthorizedPagerDutyServiceIds(scope);
            var page = await pagerDuty.GetRecentAsync(
                effectiveSince, effectiveUntil, serviceIds, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            var result = await pagerDuty.TriggerAsync(
                id, authorization.ResolveScope(context.User), cancellationToken);
            if (result is null)
            {
                return Results.Problem(
            ...
            return Results.Accepted(
                result.CaseUrl,
                new CaseTriggerResult(result.CaseId, result.CaseUrl, result.Duplicate));
        }
        catch (PagerDutyPullException exception)
        {
            loggerFactory.CreateLogger("PagerDutyPullApi").LogWarning(
