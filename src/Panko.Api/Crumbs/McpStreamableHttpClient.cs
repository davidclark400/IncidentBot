...
public sealed class McpCrumbSourceClient(
    IHttpClientFactory httpClientFactory,
    IOptions<PankoOptions> options,
    ICredentialProvider credentials,
    ILogger<McpCrumbSourceClient> logger) : IMcpCrumbSourceAdapter
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var initialize = await SendAsync(client, configuration, sessionId, new JsonObject
        {
            ...
            ["params"] = new JsonObject
            {
                ...
                ["clientInfo"] = new JsonObject { ["name"] = "panko", ["version"] = "1.0.0" }
            }
        }, cancellationToken);
    ...
    public async Task<CrumbSourceResult> CollectAsync(
        string source,
        McpToolConfiguration configuration,
        CaseContext context,
        CrumbScope scope,
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return CrumbSourceResult.Unavailable(source, 0, "MCP is disabled by feature flag.");
        }
        ...
        try
        {
            ...
            {
                ["case"] = JsonSerializer.SerializeToNode(new
                {
                    caseId = context.CaseId,
                    pagerDutyIncidentId = context.PagerDutyIncidentId,
                    context.ServiceId,
                    context.Title,
                    pagerDutyState = context.PagerDutyState.ToString(),
                    context.Labels
                }, JsonOptions),
            ...
            var sourceResult = JsonSerializer.Deserialize<CrumbSourceResult>(json, JsonOptions)
                ?? throw new InvalidOperationException("MCP Crumb source result was empty.");
            var credential = credentials.Get(configuration.CredentialEnv);
            sourceResult = McpCrumbSourceBoundary.Normalize(
                source, sourceResult, scope, context.OpenedAt, allowedBaseUrl, allowedResourceNode, credential);
            logger.LogDebug(
                "MCP collection completed for {Source} in {DurationMilliseconds} ms with {CrumbCount} Crumbs",
                source, stopwatch.ElapsedMilliseconds, sourceResult.Crumbs.Count);
            return sourceResult;
        }
