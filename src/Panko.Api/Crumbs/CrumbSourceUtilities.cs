...
internal static class CrumbSourceUtilities
{
    public static Task<CrumbSourceResult> CollectAsync(
        string source,
        ...
        Func<CancellationToken, Task<CrumbSourceResult>> collectNative,
    ...
    private static CrumbScope EffectiveMcpScope(CrumbScope scope, ConnectorTransport transport) =>
        scope with
        {
        ...
        Action<int>? observeBytesRead = null)
    {
        ...
        if (!string.IsNullOrWhiteSpace(charset))
        {
            ...
            {
                // UTF-8 is the safe default for Crumb source JSON/NDJSON responses.
            }
        }
    ...
    public static async Task<CrumbSourceResult> ExecuteAsync(
        string source,
        int timeoutSeconds,
        Func<CancellationToken, Task<CrumbSourceResult>> action,
        CancellationToken cancellationToken)
    {
...
internal sealed class CrumbSourceResponseLimitExceededException()
    : InvalidOperationException("Crumb source response exceeded its configured byte limit.");
