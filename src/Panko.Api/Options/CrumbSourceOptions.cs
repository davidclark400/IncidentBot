...
public sealed class CrumbSourceOptions
{
    public const string SectionName = "CrumbSources";

    public ConnectorTransport PagerDuty { get; init; } = new();
    public ConnectorTransport Nomad { get; init; } = new();
    public ConnectorTransport Consul { get; init; } = new();
...
public sealed record ConfiguredCrumbSource(
    string Source,
    ConnectorTransport Transport,
    IReadOnlyList<string> RecipeIds);

public sealed class CrumbSourceOptionsValidator : IValidateOptions<CrumbSourceOptions>
{
    public ValidateOptionsResult Validate(string? name, CrumbSourceOptions options)
    {
        ...
        {
            failures.Add("CrumbSources:Kafka:Mode must be 'api'; Kafka MCP transport is not supported.");
        }

        if (!IsHttpUrl(options.PagerDuty.BaseUrl))
        {
            failures.Add("CrumbSources:PagerDuty:BaseUrl must be an absolute HTTP(S) URL because incident pull uses the native API.");
        }
        if (options.PagerDuty.Mode == "mcp" && !CredentialVariableName.IsValid(options.PagerDuty.CredentialEnv))
        {
            failures.Add("CrumbSources:PagerDuty:CredentialEnv must name the native API credential because incident pull always uses the native API.");
        }
