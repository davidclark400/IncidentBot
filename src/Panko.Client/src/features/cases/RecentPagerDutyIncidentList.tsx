...
type RecentPagerDutyIncidentListProps = {
  incidents: RecentPagerDutyIncident[]
  loading: boolean
  onOpen: (id: string) => Promise<void>
  scopeLabel: string
  openError: { id: string; message: string } | null
  openingId: string | null
}

export function RecentPagerDutyIncidentList({ incidents, loading, onOpen, scopeLabel, openError, openingId }: RecentPagerDutyIncidentListProps) {
  if (loading && incidents.length === 0) return <PagerDutyIncidentListSkeleton />
  if (incidents.length === 0) return <EmptyPagerDutyIncidentList scopeLabel={scopeLabel} />
    ...
    <div className={loading ? 'opacity-60' : ''} aria-busy={loading}>
      ...
      <div className="divide-y divide-border">
        {incidents.map((incident) => {
          const rowError = openError?.id === incident.id ? openError.message : null
          ...
          return (
            <article key={incident.id} className="grid gap-4 px-4 py-5 md:grid-cols-[minmax(0,2.4fr)_minmax(9rem,1fr)_8rem_minmax(9rem,1fr)_9rem] md:items-center md:gap-5 md:px-5 md:py-4">
              ...
              <div>
                ...
                <PagerDutyIncidentStatus status={incident.status} />
              </div>
              ...
              <div>
                <button
                  ...
                  disabled={openingId !== null || loading}
                  aria-describedby={rowError ? `open-error-${incident.id}` : undefined}
                  onClick={() => void onOpen(incident.id)}
                >
                  {isOpening ? 'Opening…' : 'Open case'}
                </button>
              </div>
              {rowError && <p id={`open-error-${incident.id}`} role="alert" className="text-xs leading-5 text-rose-600 dark:text-rose-300 md:col-span-5">{rowError}</p>}
            </article>
          )
        })}
      </div>
...
function PagerDutyIncidentStatus({ status }: { status: string }) {
...
function EmptyPagerDutyIncidentList({ scopeLabel }: { scopeLabel: string }) {
  return (
    <div className="grid min-h-72 place-items-center px-6 py-12 text-center">
      <div>
        ...
        <h2 className="mt-4 text-base font-semibold text-foreground">No PagerDuty incidents for {scopeLabel}</h2>
        <p className="mt-1 text-sm text-muted-foreground">Choose a longer period or refresh to pull the latest incidents for this scope.</p>
      </div>
    </div>
  )
}

function PagerDutyIncidentListSkeleton() {
  return (
    <div aria-label="Loading recent PagerDuty incidents" className="divide-y divide-border">
      {[0, 1, 2, 3].map((item) => (
