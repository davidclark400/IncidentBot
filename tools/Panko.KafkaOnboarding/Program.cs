...
public static class KafkaOnboardingCommand
{
    ...
    private static int GenerateDashboard(CliOptions options, TextWriter output, TextWriter error)
    {
        ...
        var outputPath = options.Required("output");
        var scope = KafkaRecipeScopeLoader.Load(recipesPath, recipeId);
        ...
        if (options.Check && !generator.Check(outputPath, recipeId, plan, out var diagnostic))
        {
        ...
        var json = generator.Generate(recipeId, plan);
    ...
    private const string Usage = """
        Panko Kafka onboarding (offline; makes no live calls)
        ...
        validate --inventory <json> --recipes <yaml> --recipe-id <id> --metric-packs <yaml> --dashboard <json> [--mappings <yaml>]
        """;
