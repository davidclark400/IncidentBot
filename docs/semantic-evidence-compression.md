...
`SemanticEvidenceCompressor` is applied only while `LiteLlmSynthesizer` builds its bounded digest.
Connector results, stored reports, timelines and evidence APIs continue to use the complete source
evidence. The compressor is stateless and reused directly by the digest builder rather than being
registered with dependency injection.

## Invariants

- Original `ConnectorResult` and `EvidenceFinding` objects are never mutated.
- Exact duplicate source/ID pairs are canonicalized before semantic grouping.
- Every group retains all original member evidence IDs for audit and future citation validation.
- Up to three deterministic representatives retain source wording, punctuation, excerpts and URLs.
- Code references retain the evidence ID that owned them.
- First-observed log errors and GitLab change/deployment events remain independently citable.
- Reversing connector or finding order produces the same groups and representatives.
- Unknown sources use the conservative `preserve` policy.

## Source policies

| Source | Compressed evidence | Preserved evidence |
| --- | --- | --- |
...
| PagerDuty | Nothing beyond exact duplicate-ID canonicalization | Every incident finding |
...
Each `SemanticEvidenceGroup` contains:
...
- every member evidence ID;
- bounded code references with their owning evidence IDs.
...
into the digest. Continue evaluating captured incidents for payload size, unsupported-claim rate,
