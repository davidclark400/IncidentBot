...
public sealed class KafkaCrumbSourceTests
{
    ...
    public async Task ParsesReducersThresholdsTimestampsAndStableIds()
    {
        ...
        var recipeScope = new KafkaRecipeScope
        {
        ...
        var first = await connector.CollectAsync(Context(recipeScope), Scope(), CancellationToken.None);
        var second = await connector.CollectAsync(
            Context(recipeScope),
            Scope(TriggeredAt.AddHours(-2), TriggeredAt.AddHours(-1)),
            CancellationToken.None);

        Assert.Equal(CrumbSourceHealth.Complete, first.Health);
        Assert.Equal(2, first.Crumbs.Count);
        var lag = Assert.Single(first.Crumbs, crumb => crumb.Category == "kafka-consumer-lag");
        ...
        var context = Assert.Single(first.Crumbs, crumb => crumb.Category == "kafka-topic-rate");
        ...
        Assert.Single(first.Trail);
        var responderLink = Assert.Single(first.Links);
        Assert.Contains("/d/panko-kafka-orders-production", responderLink.Url, StringComparison.Ordinal);
        Assert.DoesNotContain("var-", responderLink.Url, StringComparison.Ordinal);
        Assert.Equal(
            first.Crumbs.OrderBy(item => item.Category).Select(item => item.Id),
            second.Crumbs.OrderBy(item => item.Category).Select(item => item.Id));
    }
    ...
    public async Task EveryFailedBatchReturnsUnavailable()
    {
        ...
        Assert.Equal(2, calls);
        Assert.Equal(CrumbSourceHealth.Unavailable, result.Health);
        Assert.Empty(result.Crumbs);
        Assert.Contains("batch 1 failed", result.Diagnostic, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("batch 2 failed", result.Diagnostic, StringComparison.OrdinalIgnoreCase);
    }

    private static KafkaCrumbSource Connector(
        ...
        TestConfiguration.CrumbSources(kafka: new Panko.Api.Options.ConnectorTransport
        {
            Mode = "api",
    ...
    private static CaseContext Context(KafkaRecipeScope kafka) => new(
        ...
        new Recipe { Id = "orders-production", Kafka = kafka });

    private static CrumbScope Scope() => Scope(TriggeredAt.AddMinutes(-30), TriggeredAt.AddMinutes(10));

    private static CrumbScope Scope(DateTimeOffset start, DateTimeOffset end) =>
    ...
    private static KafkaMetricCatalog Catalog(params MetricSpec[] metrics)
    {
        ...
        foreach (var metric in metrics)
        {
            ...
            yaml.AppendLine($"        crumbMode: {metric.CrumbMode}");
    ...
    private static MetricSpec Metric(
        ...
        double warning,
        double critical) =>
        new(id, expression, resourceScope, reducer, crumbMode, requirement, warning, critical);
