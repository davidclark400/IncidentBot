...
public sealed class GitLabCrumbSourceTests
{
    [Fact]
    public async Task MultiJobPipelinePaginatesRanksCollapsesAndBudgetsTraces()
    {
        ...
        var jobCrumbs = result.Crumbs
            .Where(crumb => crumb.Category == "pipeline-job-output")
            .ToList();

        Assert.Equal(CrumbSourceHealth.Partial, result.Health);
        Assert.Equal(20, result.Crumbs.Count);
        Assert.True(result.Trail.Count <= 20);
        ...
        Assert.Equal(4, jobCrumbs.Count);
        Assert.DoesNotContain(jobCrumbs, crumb => crumb.Summary.Contains("recovered-test", StringComparison.Ordinal));

        var hardFailure = jobCrumbs[0];
        ...
        Assert.Contains("BOUNDED-SCAN-CRUMB", hardFailure.Excerpt, StringComparison.Ordinal);
        ...
        var cascadingFailure = jobCrumbs[1];
        ...
        var allowedFailure = jobCrumbs[2];
        ...
        Assert.DoesNotContain(jobCrumbs, crumb => crumb.ObjectId is not null
            && crumb.ObjectId.StartsWith("10", StringComparison.Ordinal));

        Assert.Equal(3, handler.TraceRanges.Count);
        Assert.All(handler.TraceRanges, range => Assert.Equal("bytes=-20000", range));
        Assert.Equal(3, jobCrumbs.Count(crumb => crumb.ObjectType == "pipeline-job"));
    ...
    public async Task ChildPipelineIsCollectedAndUnverifiedPartialTraceFallsBackToRealTail()
    {
        ...
        var job = Assert.Single(result.Crumbs,
            crumb => crumb.Category == "pipeline-job-output");
        Assert.True(handler.RequestedChildPipelines);
        Assert.Equal(2, handler.TraceRequestCount);
        Assert.Equal(CrumbSourceHealth.Partial, result.Health);
        Assert.Contains("BOUNDED-FALLBACK-CRUMB", job.Excerpt, StringComparison.Ordinal);
    ...
    private static GitLabCrumbSource CreateConnector(HttpMessageHandler handler) =>
        new(
            new StubHttpClientFactory(handler),
            new UnexpectedMcpAdapter(),
            TestConfiguration.CrumbSources(gitLab: Transport()),
            TestConfiguration.Credentials());

    private static CaseContext Context()
    {
        return new CaseContext(
            Guid.NewGuid(), "PD-1", "payments", "Checkout failures", "high", PagerDutyIncidentState.Triggered,
    ...
    private static CrumbScope Scope(int maxItems, int maxBytes) => new(
        ...
        protected override HttpResponseMessage Handle(HttpRequestMessage request)
        {
            ...
            if (path.EndsWith("/jobs/401/trace", StringComparison.Ordinal))
            {
                ...
                var trace = "actual trace\n" + new string('y', 2700)
                    + "\nBOUNDED-FALLBACK-CRUMB" + new string('z', 3000) + "\nREAL-FAILURE-END";
                return new HttpResponseMessage(HttpStatusCode.OK)
                {
                    Content = new StringContent(trace, Encoding.UTF8, "text/plain")
                };
    ...
    private sealed class UnexpectedMcpAdapter : IMcpCrumbSourceAdapter
    {
