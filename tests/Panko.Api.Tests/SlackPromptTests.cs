...
public sealed class SlackQueryPlanningTests
{
    ...
    public void CompilerNarrowsReviewedQueriesAndEmitsCanonicalYaml()
    {
        ...
        Assert.Null(compiled.Recipe.PagerDuty);
        Assert.Null(compiled.Recipe.VictoriaLogs);
        var query = Assert.Single(compiled.Recipe.Grafana!.Queries);
        Assert.Equal("HTTP 5xx rate", query.Name);
        Assert.Equal("http-5xx-rate", query.MetricId);
        Assert.Equal("errors", query.Role);
        Assert.Equal("context", query.CrumbMode);
        Assert.Equal("required", query.Requirement);
        Assert.Empty(compiled.Recipe.Grafana.Dashboards);
        Assert.Empty(compiled.Recipe.Grafana.AnnotationTags);
        Assert.Contains("recipeId: payments-production", compiled.AuditYaml, StringComparison.Ordinal);
    ...
    [Fact]
    public void CompilerRetainsReviewedAnchorPatternsForSelectedVictoriaLogsQuery()
    {
        var compiled = new SlackQueryPlanCompiler(new SafeTemplateRenderer()).Compile(
            ValidPlan(queryNames: ["Errors"], source: "victorialogs"),
            Recipe());

        var query = Assert.Single(compiled.Recipe.VictoriaLogs!.Queries);
        var anchor = Assert.Single(query.AnchorPatterns);
        Assert.Equal("Payment timeout", anchor.Name);
        Assert.Equal("(?i)payment timed out", anchor.Pattern);
        Assert.Null(compiled.Recipe.Grafana);
        Assert.DoesNotContain("Payment timeout", compiled.AuditYaml, StringComparison.Ordinal);
        Assert.DoesNotContain("payment timed out", compiled.AuditYaml, StringComparison.Ordinal);
    }
    ...
    public void CompilerFailsClosedForUnknownQueriesUnsafeLabelsAndEmptyQuerySelection()
    {
        ...
        Assert.Throws<InvalidOperationException>(() => compiler.Compile(
            ValidPlan(queryNames: ["not-reviewed"]), recipe));
        Assert.Throws<InvalidOperationException>(() => compiler.Compile(
            ValidPlan(labels: [new SlackQueryLabel("environment", "production\" OR 1=1")]), recipe));
        Assert.Throws<InvalidOperationException>(() => compiler.Compile(
            ValidPlan(labels: [new SlackQueryLabel("environment", "staging")]), recipe));
        Assert.Throws<InvalidOperationException>(() => compiler.Compile(
            ValidPlan(queryNames: []), recipe));
        Assert.Throws<InvalidOperationException>(() => compiler.Compile(
            ValidPlan(source: "pagerduty", queryNames: []), recipe));
    }

    [Fact]
    public void KafkaSelectionKeepsReviewedScopeButLeaksNoPackOrPromQlIntoPlanCatalog()
    {
        var reviewed = Recipe().WithKafka(new KafkaRecipeScope
        {
        ...
        Assert.NotNull(compiled.Recipe.Kafka);
        Assert.Equal(["payments"], compiled.Recipe.Kafka.Topics);
        Assert.Null(compiled.Recipe.Grafana);
        Assert.Null(compiled.Recipe.VictoriaLogs);
    ...
    [Fact]
    public void ConsulSelectionKeepsOnlyTheReviewedServiceScope()
    {
        var compiled = new SlackQueryPlanCompiler(new SafeTemplateRenderer()).Compile(
            ValidPlan(source: "consul", queryNames: []),
            Recipe());

        var consul = Assert.IsType<ConsulScope>(compiled.Recipe.Consul);
        var service = Assert.Single(consul.Services);
        Assert.Equal("payments-api", service.Name);
        Assert.Equal("payments", service.Namespace);
        Assert.Null(compiled.Recipe.Grafana);
        Assert.Contains("source: consul", compiled.AuditYaml, StringComparison.Ordinal);
        Assert.DoesNotContain("payments-api", compiled.AuditYaml, StringComparison.Ordinal);
    }
    ...
    public async Task PlannerUsesDedicatedModelAndSendsOnlySafeCatalogNames()
    {
        ...
        var result = await planner.PlanAsync("Are errors rising?", Recipe(), CancellationToken.None);
        ...
        Assert.DoesNotContain("payment timed out", serializedRequest, StringComparison.Ordinal);
    }
    ...
    internal static Recipe Recipe() => new()
    {
        ...
        {
            Datacenter = "primary",
            Services = [new ConsulService { Name = "payments-api", Namespace = "payments" }]
        },
...
file static class SlackKafkaRecipeExtensions
{
    public static Recipe WithKafka(
        this Recipe recipe,
        KafkaRecipeScope kafka) => new()
        {
            Id = recipe.Id,
            PagerDutyServiceId = recipe.PagerDutyServiceId,
            Team = recipe.Team,
            SlackChannel = recipe.SlackChannel,
            SlackPromptLabels = recipe.SlackPromptLabels,
            Selectors = recipe.Selectors,
            PagerDuty = recipe.PagerDuty,
            Nomad = recipe.Nomad,
            Consul = recipe.Consul,
            GitLab = recipe.GitLab,
            Grafana = recipe.Grafana,
            Kafka = kafka,
            VictoriaLogs = recipe.VictoriaLogs
        };
}
...
public sealed class SlackPromptWorkflowTests
{
    ...
    public async Task MentionRunsPlannerSelectedDatasourceExistingSynthesisAndOneThreadedReply()
    {
        ...
        Assert.Contains("Crumb-grounded answer", reply.Text, StringComparison.Ordinal);
        ...
        Assert.Equal("audit", harness.Operations[0]);
        Assert.Equal("planner", harness.Operations[1]);
        var auditEvent = Assert.Single(harness.Audit.Events);
        Assert.Equal(SecurityAuditActions.SlackPrompt, auditEvent.Action);
        ...
        Assert.Equal("C123", auditEvent.Metadata!["channel_id"]);
        Assert.Equal("Ev123", auditEvent.Metadata["slack_event_id"]);
        Assert.DoesNotContain(
            mention.Prompt,
            string.Join("\n", auditEvent.Metadata.Values),
    ...
    public async Task UnmappedChannelMakesNoPlannerOrDatasourceCalls()
    {
        ...
        Assert.Null(auditEvent.TargetTeam);
    }

    [Fact]
    public async Task RecipeOwnedByAnotherTeamIsAuditedAndRejectedBeforePlanning()
    {
        var harness = CreateHarness(
            SlackQueryPlanningTests.ValidPlan(),
            channelTeam: "orders");
        ...
        Assert.Equal(0, harness.Planner.Calls);
        Assert.Equal(0, harness.Grafana.Calls);
        Assert.Equal(["audit"], harness.Operations);
        var auditEvent = Assert.Single(harness.Audit.Events);
        Assert.Equal(SecurityAuditActions.SlackPrompt, auditEvent.Action);
        Assert.Equal("denied", auditEvent.Outcome);
        Assert.Equal(["orders"], auditEvent.Actor.Teams);
    ...
    private static Harness CreateHarness(
        SlackQueryPlan plan,
        bool grafanaMcp = false,
        string channelTeam = "payments")
    {
        var recipe = SlackQueryPlanningTests.Recipe();
        var operations = new List<string>();
        var planner = new RecordingPlanner(plan, operations);
        var grafana = new RecordingConnector("grafana");
        var victoriaLogs = new RecordingConnector("victorialogs");
        var sourceConfiguration = TestConfiguration.CrumbSources(
            grafana: grafanaMcp
                ? new ConnectorTransport
                {
                    ...
                    Mcp = new McpToolConfiguration
                    {
                        ...
                        ToolName = "collect_crumbs",
                        CredentialEnv = "MCP_TOKEN"
                    }
                }
                : null);
        var registry = new CrumbSourceRegistry(
        ...
        var collector = new AdaptiveCrumbCollector(
            Microsoft.Extensions.Options.Options.Create(new PankoOptions
            {
                ...
                CrumbMaximumItems = 50,
                CrumbMaximumBytes = 65536
            }),
            time,
            NullLogger<AdaptiveCrumbCollector>.Instance);
        var synthesizer = new RecordingSynthesizer();
        var recipeProvider = new RecipeProvider(recipe);
        var caseQueryRunner = new SlackCaseQueryRunner(
            recipeProvider,
            planner,
            ...
            synthesizer,
            time,
            NullLogger<SlackCaseQueryRunner>.Instance);
        ...
        var handler = new SlackMentionHandler(
            ...
            Microsoft.Extensions.Options.Options.Create(new SlackOptions
            {
                ...
                PromptChannelRecipes = new Dictionary<string, string>
                {
    ...
    private sealed class RecipeProvider(Recipe recipe) :
        ISlackQueryRecipeProvider,
        IRecipeOwnershipCatalog
    {
        public IReadOnlyList<RecipeOwnership> All =>
            [new(recipe.Id, recipe.Team, recipe.PagerDutyServiceId)];

        public bool TryGet(string recipeId, out RecipeOwnership ownership)
        {
            if (string.Equals(recipeId, recipe.Id, StringComparison.Ordinal))
            {
                ownership = new RecipeOwnership(recipe.Id, recipe.Team, recipe.PagerDutyServiceId);
                return true;
            }
        ...
        public SlackQueryRecipe Resolve(string recipeId)
        {
            ...
            return new SlackQueryRecipe("test-revision", recipe);
        }
    }

    private sealed class RecordingPlanner(
        SlackQueryPlan plan,
        List<string>? operations = null) : ISlackQueryPlanner
    {
    ...
    private sealed class RecordingConnector(string source) : ICrumbSourceAdapter
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            {
                "grafana" => context.Recipe.Grafana?.Queries.Select(query => query.Name).ToArray() ?? [],
                "victorialogs" => context.Recipe.VictoriaLogs?.Queries.Select(query => query.Name).ToArray() ?? [],
                _ => []
            };
            var crumb = new Crumb(
                $"{Source}-crumb",
