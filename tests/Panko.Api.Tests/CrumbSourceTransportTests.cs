...
public sealed class ConnectorTransportTests
{
    ...
    public async Task McpModeUsesTheMcpAdapter()
    {
        ...
        var result = await CrumbSourceUtilities.CollectAsync(
            "source",
    ...
    private static CaseContext Context() => new(
        Guid.NewGuid(), "PD-1", "payments", "Case", "high", PagerDutyIncidentState.Triggered,
    ...
    private static CrumbSourceResult Result(string source) =>
        new(source, CrumbSourceHealth.Complete, [], [], [], 0, null);

    private sealed class RecordingMcpAdapter : IMcpCrumbSourceAdapter
    {
        public bool WasCalled { get; private set; }
        public CrumbScope? ReceivedScope { get; private set; }
