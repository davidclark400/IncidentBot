...
public sealed class KafkaMetricCatalogTests
{
    ...
    public void SharedSyntheticPackIsVersionedAndComplete()
    {
        ...
        Assert.All(plan.Metrics, metric =>
        {
            ...
            Assert.Contains("panko_fixture_kafka_", metric.RuntimePromQl, StringComparison.Ordinal);
    ...
    public void MultipleFullyScopedSelectorsAreSupported()
    {
        ...
        var plan = catalog.CompilePlan(new KafkaRecipeScope
        {
    ...
    [Fact]
    public void RecipeRequiresKnownOverridesAndAnAllowlistedTopic()
    {
        var catalog = SharedCatalog();
    ...
    public void RequiredConsumerGroupMetricRequiresAConsumerGroupAllowlist()
    {
        var catalog = KafkaMetricCatalog.Parse("""
            ...
            """);
        var scope = new KafkaRecipeScope
        {
            MetricPackId = "pack-v1",
            Cluster = "production",
            Topics = ["orders"],
    ...
    internal static KafkaRecipeScope Scope(
...
{
    public static KafkaRecipeScope WithTopics(this KafkaRecipeScope scope, List<string> topics) => new()
    {
