...
public sealed class KafkaResourceMappingTests
{
    ...
    [Fact]
    public void OrphanMappingAndTargetOutsideRecipeScopeFail()
    {
        var scope = KafkaMetricCatalogTests.Scope();
        var catalog = KafkaMetricCatalogTests.SharedCatalog();
        var dashboard = new KafkaDashboardGenerator().Generate(
        ...
        var mappings = KafkaResourceMappingLoader.Parse("""
            ...
            mappings:
              ...
              - kind: topic
                ...
                recipeResource: unreviewed-topic
                evidenceFile: exporter/catalog.yaml
                evidenceLine: 47
            """);
        ...
        Assert.Contains(result.Errors, error => error.Contains("outside the Recipe allowlist", StringComparison.Ordinal));
    }
